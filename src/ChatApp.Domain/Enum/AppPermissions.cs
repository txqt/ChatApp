...
namespace ChatApp.Domain.Enum
{
    ...
    public enum AppPermissions : long
    {
        ...
        EditOwnProfile = 1L << 5,     // Chỉnh sửa thông tin cá nhân

        // ===== SYSTEM MANAGEMENT =====
        ViewSystemLogs = 1L << 6,     // Xem log hệ thống
        ManageSystem = 1L << 7,       // Quản lý cấu hình hệ thống
        ManageRoles = 1L << 8,        // Quản lý role toàn hệ thống
        ViewAnalytics = 1L << 9,      // Xem thống kê hệ thống

        // ===== GLOBAL CHAT MANAGEMENT =====
        CreateDirectChat = 1L << 10,      // Tạo group chat mới
        CreateGroup = 1L << 11,           // Tạo group chat mới
        ViewAllChats = 1L << 12,          // Xem tất cả chat
        DeleteAnyChat = 1L << 13,         // Xóa bất kỳ chat nào

        // ===== PREDEFINED COMBINATIONS =====
        BasicUser = CreateDirectChat | CreateGroup | EditOwnProfile,
